...
"""def add_to_chroma(chunks: list[Document]):
    ...
    if len(new_chunks):
        ...
        # No need to call db.persist() as persistence is handled automatically
    else:
...
def add_to_chroma(chunks: list[Document]):
    try:
        ...
        if new_chunks:
            ...
            # Add documents in batches to avoid exceeding maximum batch size.
            max_batch_size = 5461
            for i in range(0, len(new_chunks), max_batch_size):
                batch_chunks = new_chunks[i:i + max_batch_size]
                batch_chunk_ids = new_chunk_ids[i:i + max_batch_size]
